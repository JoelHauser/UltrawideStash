...
{
    /// <summary>One profile's stash, as read.</summary>
    ...
    public static StashContents? Read(string path, Func<string, (int Width, int Height)> sizeOf)
    {
        ...
        foreach (var node in items)
        {
            ...
            if (node["parentId"]?.GetValue<string>() != stashId) continue;
            ...
            placements.Add(new StashRepack.Placement(
                id!,
                ReadInt(loc, "x"),
                ReadInt(loc, "y"),
                size.Width,
        ...
        return new StashContents(path, stashId!, stashTemplate!, placements);
    }

    /// <summary>
    /// Apply moves to a profile on disk. Returns how many items were actually written.
    ///
    ...
    /// last step.
    /// </summary>
    public static int ApplyMoves(string path, IReadOnlyList<StashRepack.Move> moves)
    {
        ...
        var wanted = new Dictionary<string, StashRepack.Move>(moves.Count);

        foreach (var move in moves) wanted[move.ItemId] = move;
        ...
        foreach (var node in items)
        {
            ...
            if (id is null || !wanted.TryGetValue(id, out var move)) continue;
